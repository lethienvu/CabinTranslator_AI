...
class App {
    ...
    async init() {
        ...
        // About tab (links removed)
        this._initAboutTab();

        console.log('🌐 Cabin Translator v0.5.0 initialized');
    }
    ...
    _bindEvents() {
        ...
        // Click outside to close format overlay
        document.addEventListener('click', (e) => {
            const controls = document.querySelector('.floating-controls');
            const formatBtn = document.getElementById('btn-format');
            if (controls && controls.classList.contains('show')) {
                if (!controls.contains(e.target) && (!formatBtn || !formatBtn.contains(e.target))) {
    ...
    _updateStartButton() {
        ...
        if (label) {
            label.textContent = this.isRunning ? 'Dừng' : 'Bắt đầu';
        }
    }
    ...
    _initAboutTab() {
        // No external links or update actions
    }
