...
Cabin Translator can **read translations aloud** as they appear — like having a personal interpreter. Three providers available:
...
Edge TTS uses the same neural speech engine behind Microsoft Edge's **"Read Aloud"** feature. Cabin Translator connects to the same service to read translations.
